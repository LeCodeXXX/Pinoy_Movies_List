  ...
  return (
    <section aria-labelledby={`genre-${section.genre.id}`}>
      <div className="mb-6 flex items-center justify-between gap-4">
        ...
        <Link
          className="inline-flex h-10 shrink-0 items-center rounded-full border border-red-500/30 bg-red-600 px-4 text-xs font-bold text-white shadow-lg shadow-red-950/30 transition hover:bg-red-500"
      ...
      {isLoading ? (
        ...
        <div className="group/carousel relative px-5 sm:px-7">
          <div
          ...
          <button
            ...
            className="absolute left-0 top-1/2 z-10 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-white/15 bg-zinc-950/95 text-xl font-bold text-white shadow-xl shadow-black/60 transition hover:scale-110 hover:border-red-400 hover:bg-red-600 disabled:cursor-not-allowed disabled:opacity-30"
          ...
          <button
            ...
            className="absolute right-0 top-1/2 z-10 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-white/15 bg-zinc-950/95 text-xl font-bold text-white shadow-xl shadow-black/60 transition hover:scale-110 hover:border-red-400 hover:bg-red-600 disabled:cursor-not-allowed disabled:opacity-30"
            disabled={isLoading || section.movies.length <= 6}
            onClick={() => scroll(1)}
            type="button"
          >
...
function GenreRowSkeleton() {
  return (
    <div className="flex gap-5 overflow-hidden px-5 sm:px-7">
      {Array.from({ length: 6 }, (_, index) => (
        <div
          key={index}
          className="aspect-[2/3] w-36 shrink-0 animate-pulse rounded-xl border border-zinc-800 bg-zinc-900 sm:w-40 lg:w-44"
        />
      ))}
    </div>
  )
